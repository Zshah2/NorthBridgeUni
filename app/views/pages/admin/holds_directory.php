      ...
      <?php foreach ($holdRows as $h): ?>
        <tr class="hover:bg-slate-50/70">
          <td class="px-4 py-3 font-mono text-xs">
            <a class="font-semibold text-indigo-700 hover:underline" href="<?= htmlspecialchars(url('/admin.php?view=people&id=' . (int)$h['student_id'])) ?>"><?= (int)$h['student_id'] ?></a>
          </td>
          <td class="px-4 py-3"><?= htmlspecialchars(trim((string)($h['first_name'] ?? '') . ' ' . (string)($h['last_name'] ?? ''))) ?></td>
